...
def plot_multiple_csv_files(csv_files):
    ...
    for csv_file in csv_files:
        ...
        for col in ['Rudder angle', 'Mast angle']:
    ...
    for idx, data in enumerate(all_data):
        ...
        # Rudder and Mast angles
        for pot_col in ['Rudder angle', 'Mast angle']:
        ...
        if is_top_left_plot and pot_handles:
            ax_pot.legend(handles=pot_handles, loc='upper right', fontsize=12, frameon=True)

        # SOG / VMG
        for colname in ['SOG', 'VMG']:
            if colname in df.columns:
                linestyle = ':' if colname == 'VMG' else '-'
                line, = ax_gps.plot(x_time, df[colname], label=colname, color=colors[colname], linestyle=linestyle)
        ...
        if course_col in df.columns:
            line, = ax_gps_2.plot(x_time, df[course_col], label="Course", color=colors['Course'])
            gps_handles.append(line)
        if wind_dir_col is not None:
            line, = ax_gps_2.plot(x_time, df[wind_dir_col], label="EOWD",
                                  color=colors['EOWD'], linestyle='--')
            gps_handles.append(line)
        ...
        if is_top_left_plot and gps_handles:
            ax_gps.legend(handles=gps_handles, loc='lower left', fontsize=12, frameon=True)
...
if __name__ == "__main__":
    csv_files1 = [
        "tacks_converted_deg/Turn_01.csv",
        "tacks_converted_deg/Turn_03.csv",
        "tacks_converted_deg/Turn_05.csv",
        "tacks_converted_deg/Turn_07.csv",
        "tacks_converted_deg/Turn_09.csv",
        "tacks_converted_deg/Turn_11.csv"
    ]
    csv_files2 = [
        "tacks_converted_deg/Turn_02.csv",
        "tacks_converted_deg/Turn_04.csv",
        "tacks_converted_deg/Turn_06.csv",
        "tacks_converted_deg/Turn_08.csv",
        "tacks_converted_deg/Turn_10.csv",
        "tacks_converted_deg/Turn_12.csv"
    ]
