...
df = pd.read_csv('stepper_rudder.csv')
plt.rcParams.update({'font.size': 18})
# Remap Pot3: max -> 0, min -> 250
pot1_min = df['Pot1'].min()
pot1_max = df['Pot1'].max()
df['Pot1_mapped'] = 270-((df['Pot1'])/4095*270)-30
...
plt.plot(df['Pot1_mapped'], label='Potentiometer')
plt.plot(df['StepperAngle'], label='Stepper')

# Add horizontal dotted lines
#plt.axhline(pot2250, color='red', linestyle='--', label='Pot1=2250 (mapped)')
#plt.axhline(pot1000, color='green', linestyle='--', label='Pot1=1000 (mapped)')
#plt.axhline(pot2500, color='blue', linestyle='--', label='Pot1=2500 (mapped)')
